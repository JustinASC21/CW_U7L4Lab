...
public class ArrayListAlgorithms {
    ...
    public static boolean containsTarget(ArrayList<String> stringList, String target)
    {
        for (int i = 0; i < stringList.size(); i ++) {
            for (int x = 0; x <= stringList.get(i).length() - target.length(); x ++) {
                if (stringList.get(i).substring(x,x+target.length()).equals(target)) {
                    return true;
                }
            }
        }
    ...
    {
        int min = intList.size() - 1;
        for (int i = intList.size() - 2; i >= 0; i --) {
            if (intList.get(i) <= intList.get(min))  {
                min = i;
...
//     *  @param numList1  arraylist of Integers
    ...
    public static ArrayList<String> parseWordsAndReverse(String sentence)
    {
        ...
        while (sentence.lastIndexOf(" ") != -1) {
            ans.add(sentence.substring(sentence.lastIndexOf(" ")));
    ...
    public static void moveBWords(ArrayList<String> wordList)
    {
        ...
        for (int i = 0; i < wordList.size(); i ++) {
            ...
            {
                String taken = wordList.remove(i);
                wordList.add(left,taken);
                left++;
            }
        }
    }
    ...
    public static ArrayList<Integer> modes(int[] numList)
    {
        ...
        for (int x = numList.length - 1; x >= 0; x --) {
            int numModes = 0;
            for (int i = x- 1; i >= 0; i--) {
            ...
            if (numModes >= overallMode) {
                overallMode = numModes;
                modes.add(numList[x]);
            }
        }
        return modes;
